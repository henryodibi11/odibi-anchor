...
def test_package_metadata_has_one_source_authority() -> None:
    ...
    assert '"0.3.14"' not in init_source
...
def _assert_runtime_metadata(probe: dict[str, object]) -> None:
    ...
    assert probe["runtime_version"] == probe["distribution_version"] == "0.3.14"
...
def _qualify_package_metadata_matrix(audit_root: Path) -> None:
    ...
    wheel = next(artifacts.glob("odibi_anchor-0.3.14-*.whl"))
    sdist = artifacts / "odibi_anchor-0.3.14.tar.gz"
    ...
    with zipfile.ZipFile(wheel) as wheel_archive:
        ...
        assert '"0.3.14"' not in packaged_init
    ...
    assert collision_result["version"] == "0.3.14"
    ...
    qualification_script = """
        ...
        run = QualificationRun(
            "synthetic-run", "public-t1-localized-fix", "1.0", "a" * 40, "0.3.14",
    ...
    assert masked_wheel_probe["runtime_version"] == "0.3.14"
    ...
    try:
        assert _runtime_probe(wheel_python, environment, cwd=audit_root)["runtime_version"] == "0.3.14"
    ...
    wheel_metadata_path = next(wheel_site_packages.glob("odibi_anchor-0.3.14.dist-info")) / "METADATA"
    ...
    assert masked_editable_probe["runtime_version"] == "0.3.14"
    ...
    try:
        conflicting_pyproject = authoritative_pyproject.replace('version = "0.3.14"', 'version = "9.9.9"', 1)
        ...
        assert nonlocal_authority_probe["runtime_version"] == "0.3.14"
        ...
        assert malformed_url_probe["runtime_version"] == "0.3.14"
    ...
    sdist_root = extracted / "odibi_anchor-0.3.14"
    ...
    sdist_wheel = next(sdist_wheelhouse.glob("odibi_anchor-0.3.14-*.whl"))
