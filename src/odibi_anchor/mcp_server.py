...
@mcp.tool()
def anchor_read_workspace_file(workspace_path: str) -> str:
    """Read a file from Databricks workspace.

    Args:
        workspace_path: Full workspace path (e.g. "/Workspace/Users/user@example.com/file.py")

    Returns:
        File content as string, or error dict if read fails.

    Examples:
        # Read Python file
        anchor_read_workspace_file("/Workspace/Users/user@example.com/script.py")

        # Read markdown
        anchor_read_workspace_file("/Workspace/Users/user@example.com/README.md")
    """
    try:
        from databricks.sdk import WorkspaceClient
        w = WorkspaceClient()

        with w.workspace.download(workspace_path) as f:
            return f.read().decode('utf-8')
    except Exception as e:
        return _fmt({
            "error": f"Failed to read {workspace_path}",
            "message": str(e),
...
@mcp.tool()
def anchor_write_workspace_file(workspace_path: str, content: str, overwrite: bool = False) -> str:
    ...
    Returns:
        Success message with path, or error dict if write fails.

    Examples:
        # Create new file
        anchor_write_workspace_file(
        ...
        anchor_write_workspace_file(
            "/Workspace/Users/user@example.com/config.json",
            '{"key": "value"}',
            overwrite=True
        )
    """
    try:
        ...
        return _fmt({
            ...
            "overwrite": overwrite,
            "message": f"✅ Written: {workspace_path}"
        })
    except Exception as e:
        return _fmt({
            "error": f"Failed to write {workspace_path}",
            "message": str(e),
            "tip": "Check write permissions and set overwrite=True if file exists"
        })


@mcp.tool()
def anchor_list_workspace_dir(workspace_path: str) -> str:
    """List files in a workspace directory.

    Args:
        workspace_path: Full workspace directory path (e.g. "/Workspace/Users/user@example.com/")

    Returns:
        JSON list of files with path and type, or error dict if list fails.

    Examples:
        # List user directory
        anchor_list_workspace_dir("/Workspace/Users/user@example.com/")

        # List project directory
        anchor_list_workspace_dir("/Workspace/Users/user@example.com/my_project")
    """
    try:
        from databricks.sdk import WorkspaceClient
        w = WorkspaceClient()

        items = list(w.workspace.list(workspace_path))
        return _fmt({
            "path": workspace_path,
            ...
            "files": [
                {
                    "path": i.path,
                    "type": i.object_type.value if i.object_type else "unknown",
                    "size": i.size if hasattr(i, 'size') else None
                }
                for i in items
            ]
        })
        ...
        return _fmt({
            "error": f"Failed to list {workspace_path}",
            "message": str(e),
            "tip": "Check that the path exists and you have read permissions"
        })


if os.environ.get("ANCHOR_MCP_GATEWAY_ONLY") == "1":
    for workspace_tool in (
        "anchor_read_workspace_file",
        "anchor_write_workspace_file",
        "anchor_list_workspace_dir",
    ):
        mcp.local_provider.remove_tool(workspace_tool)
